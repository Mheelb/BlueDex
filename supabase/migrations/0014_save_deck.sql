-- Sauvegarde atomique et validée d'un deck.
--
-- Les policies RLS ne peuvent pas vérifier des règles agrégées sur plusieurs
-- lignes (ex: "42 cartes au total") au moment où une seule ligne s'insère.
-- Cette fonction fait tout l'enregistrement (decks + deck_cards) dans une
-- seule transaction, puis vérifie la légalité du deck juste avant de
-- terminer : si une règle est violée, elle lève une exception et toute la
-- transaction est annulée (rien n'est sauvegardé). C'est le seul chemin que
-- l'app utilise pour sauvegarder un deck (cf. src/queries/decks.ts).
--
-- security invoker (comportement par défaut, précisé ici pour la lisibilité) :
-- la fonction s'exécute avec les droits de l'utilisateur connecté, donc les
-- policies RLS existantes sur decks/deck_cards (écriture réservée au
-- propriétaire) s'appliquent quand même en filet de sécurité.
...
begin
  ...
  if v_deck_id is null then
    ...
    values (auth.uid(), p_name, p_format)
    returning id into v_deck_id;
  else
    update public.decks
    set name = p_name, format = p_format, updated_at = now()
...
grant execute on function public.save_deck(uuid, text, text, jsonb) to authenticated;
